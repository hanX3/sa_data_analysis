...
Cube::Cube(char* cube_name,char  *tab_name, float Emax)
{
    ...
    if (read_tab(tab_file_name,&nclook,&lmin,&lmax,luch) ==1) {
        printf("open tab file %s error, plz check!\n",tab_file_name);
        exit(-1);
    }
    //get input largest ch
    C_adcmax = nclook;
    C_Emax=Emax;
    C_E2CH=1.0*C_adcmax/C_Emax;
    if (lmax > RW_MAXCH) {
    ...
    strcpy(C_CubeFileName,cube_file_name);
    if (!(C_CubeFile=open_3cube(cube_file_name, C_length))) 
    {
    ...
    C_ScrSize=500;
    if(!(C_ScrFile = open_scr(scr_file_name, C_ScrSize)))
    {
        fclose(C_CubeFile);
...
FILE* Cube::open_3cube (char *name, int length)
{
    ...
    C_nummc = nummc;
    C_numrecs = (nummc + 4087)/4088; /* note, an empty minicube is 1 byte */
    //file head 1024byte
    strncpy(head.id, "Incub8r3/Pro4d  ", 16);
    head.numch = length;
    head.bpc = 4;
    head.cps = 6;
    head.numrecs = C_numrecs;
...
void Cube::Save()
{
    ...
    numchunks = (C_nummc+nmcperc-1)/nmcperc;

    printf("Updating cube from scratch file\n");

    /* read in first record */
    recnumIn = 0;
    fseek(C_CubeFile, 1024, SEEK_SET);
    if (!fread(&recIn, 4096, 1, C_CubeFile)) {
    ...
    for (chunknum=0; chunknum<numchunks; chunknum++) 
    {
        ...
        while (scrptr[addr8b]>0) {
            fseek(C_ScrFile, ((long)scrptr[addr8b]-1)*RW_SCR_RECL, SEEK_SET);
            if (!fread(inbuf, RW_SCR_RECL, 1, C_ScrFile)) {
    ...
    recOut.h.nummc = C_nummc - recOut.h.minmc;
    if (!fwrite(&recOut, 4096, 1, OutFile)) {
        printf("Cannot write cube, aborting...\n");
        exit(-1);
    }
    ...
    fclose(C_CubeFile);
    C_CubeFile = OutFile;
    if (rename(filname, C_CubeFileName)) {
...
void Cube::dbuf2(int addr8b)
{
    ...
    if (!fwrite(outbuf,RW_SCR_RECL,1,C_ScrFile)) 
    {
        printf(" Could not write to scr file.. fatal.\n");
        exit(-1);
    }
    C_numscr++;
    scrptr[addr8b] = C_numscr;

    if (C_numscr*RW_SCR_RECL >= C_ScrSize*1024*1024)
    {
        /* scr full, inc cub */
        printf("\n  Scratch File full...\n");
        Save();
...
{
    int ch=C_E2CH*E+1.0*rand()/RAND_MAX;
    if(ch>=C_adcmax)
        ch=C_adcmax-1;
    else if(ch<0)
        ch=0;
...
void Cube::Fill(int gemult,int* elist)
{
    ...
    for (i=0; i<gemult; i++) 
    {
        if (elist[i] >= 0 && elist[i] < C_adcmax && luch[elist[i]]>0)
